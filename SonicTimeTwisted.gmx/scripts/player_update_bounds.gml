...
case 0:
    ...
    if y>(camera.bottom+offset_y) {play_sfx(sndHurt); player_is_dead(); return false;}
...
case 90:
    ...
    if x>(camera.right+offset_y) {play_sfx(sndHurt); player_is_dead(); return false;}
    break;
case 180:
    if x<(camera.left+offset_x) {x = camera.left+offset_x; if (xspeed>0) xspeed = 0;}
    if x>(camera.right-offset_x) {x = camera.right-offset_x; if (xspeed<0) xspeed = 0;}
    //if y<(camera.top-offset_y) {play_sfx(sndHurt); player_is_dead(); return false;}
    break;
case 270:
    //if y<(camera.top+offset_x) {y = camera.top+offset_x; if (xspeed<0) xspeed = 0;}
    if y>(camera.bottom-offset_x) {y = camera.bottom-offset_x; if (xspeed>0) xspeed = 0;}
    if x<(camera.left-offset_y) {play_sfx(sndHurt); player_is_dead(); return false;}
    break;
}
