...
if(objProgram.device_info & DEVICE_OS_ANDROID) {
    ...
    if(sttandroid_mode_get() && (string_length(currentDeviceLabel) > 0))
    {
        ...
        menu_fn_add_option(tr("_options_menu_Gamepad_analog_controls"), 34);
    ...
    var currentDeviceLabel = objProgram.inputManager.paddesc;
    
    if(objProgram.inputManager.pad > -1 && (string_length(currentDeviceLabel) > 0))
    {
        menu_fn_add_option(tr("_btusb_joymap_Device"), 31);
        menu_fn_add_option(tr("_btusb_joymap_Rumble"), 0, 1, "< " + tr("_Off") + " >");
