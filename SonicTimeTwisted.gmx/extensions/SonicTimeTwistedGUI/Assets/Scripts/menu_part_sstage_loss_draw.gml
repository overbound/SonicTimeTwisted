if(state == 1)
{
    ...
    if(timer < 30)
    {
        draw_text(internal__draw_menu_rect_x_center + 16 * (30 - timer),
            internal__draw_menu_rect_y_center,
            items[0, 0]);
    }
    else
    if(timer >= 30 && timer < 330)
    {
    ...
    if(timer >= 330 && timer < 360)
    {
        draw_text(internal__draw_menu_rect_x_center - 16 * (timer - 330),
            internal__draw_menu_rect_y_center,
            items[0, 0]);
    }
    ...
    {
        var height_offset;
        height_offset = round((internal__draw_menu_rect_y2 - internal__draw_menu_rect_y1) / 20 * (10 - (timer - 360)));
        draw_set_alpha(0.75);
